...
from app.services.research_services import create_research, get_research_sessions, get_research_by_id
...
def get_research_list(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    return get_research_sessions(db, current_user)
